def bad_patterns(line, index):
    ...
    dictionary = {
        ...
        'increases as temperature is increased': 'The phrase "increases as temperature is increased" can be simplified as "increases with temperature.',
        'increase as temperature is increased': 'The phrase "increase as temperature is increased" can be simplified as "increase with temperature.',
        'increase as temperature is increased': 'The phrase "increase as temperature is increased" can be simplified as "increase with temperature.',
        'increase as temperature was increased': 'The phrase "increase as temperature was increased" can be simplified as "increased with temperature.',
        'increased as temperature was increased': 'The phrase "increased as temperature was increased" can be simplified as "increased with temperature.',
    ...
    for word in dictionary:
        if word in line:
            ...
            mistakes += str('<p>'+ 'Line ' + str(index + 1) + '. ' + str(dictionary[word]) +'</p>')
    return mistakes
...
def phrases_with_very(line, index):
    ...
    for word in dictionary:
        if word in line:
            mistakes += str('<p>Line ' + str(index + 1) + '. Consider replacing "' + word + '" with words like ' + dictionary[word] + ' etc. </p>')
            #output(str('Line ' + str(index + 1) + '. Consider replacing "' + word + '" with words like ' + dictionary[word] + ' etc.'))
    return mistakes


def start_with_numbers(line, index):
    '''Check if the sentence starts with a number, but only if it is not an empty line'''
    mistakes = ''
    if len(line) > 5:
        if line[0].isdigit():
...
def figure_references(line, index):
    '''Check for 'Fig.' in the beginning of the line or 'Figure' in the middle, but only if it is not an empty line'''
    mistakes = ''
    if len(line) > 5:
